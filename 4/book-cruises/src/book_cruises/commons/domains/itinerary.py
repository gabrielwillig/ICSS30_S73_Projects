...
class Itinerary(BaseModel):
    ...
    def __init__(self, **data):
        ...
        if isinstance(self.departure_date, date):
            self.departure_date = self.departure_date.isoformat()  # Convert date to ISO format string
        if isinstance(self.arrival_date, date):
            self.arrival_date = self.arrival_date.isoformat()  # Convert date to ISO format string
